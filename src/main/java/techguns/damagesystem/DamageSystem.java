...
import techguns.damagesystem.damageHandler.DamageHandlerFactory;
...
import techguns.packets.PacketPlayCriticalSoundTarget;
...
public class DamageSystem {
    ...
    public static void playerShootProjectileDamageHit(LivingHurtEvent event,EntityLivingBase shooter,int level){
        float actualDamage = DamageHandlerFactory.getHandler().execute(event,shooter,level);
        TGDamageSource source = (TGDamageSource) event.getSource();
        if (Boolean.FALSE.equals(source.getCriticalHitFlat())){
            event.setAmount(actualDamage);
        }
        EntityLivingBase mob = event.getEntityLiving();
        float mobHealth = mob.getHealth();
        actualDamage = Math.max(actualDamage - mob.getAbsorptionAmount(), 0.0F);
        mob.setAbsorptionAmount(mob.getAbsorptionAmount() - (mobHealth - actualDamage));
        if (actualDamage!=0f){
            mob.setHealth(mobHealth - actualDamage);
            mob.getCombatTracker().trackDamage(event.getSource(), mobHealth, actualDamage);
            mob.setAbsorptionAmount(mob.getAbsorptionAmount() - actualDamage);
        }
    }
