...
//    }
    ...
    public TGExplosion(World world, Entity exploder, Entity projectile, double x, double y, double z, double primaryDamage, double secondaryDamage, double primaryRadius, double secondaryRadius, double blockDamageFactor, ItemStack itemStack, float pen,String damageUuid)
    {
        ...
        for (int k2 = 0; k2 < list.size(); ++k2)
        {
            ...
            if (!entity.isImmuneToExplosions() && GenericProjectile.BULLET_TARGETS.apply(entity))
            {
	            ...
	            if (damage > 0.0) {
                    ...
                    float dmg = (float) damage * f;
	            	//System.out.println("Attack Damage: "+ damage +" against "+entity);
                    tgs.setOriginalDamage((float)Math.max(0, dmg));
                    tgs.setSourceItemStack(itemStack);
                    tgs.setPenetration(this.pen);
                    tgs.setDamageUuid(this.damageUuid);
                    tgs.setPlaySoundNextDerivativeBullet(Boolean.FALSE);
                    tgs.attacker = this.exploder;

                    if (this.itemStack==null){
                        entity.attackEntityFrom(tgs,  (float)Math.max(0, dmg));
                        return;
                    }
                    ...
                    if (enchantmentList!=null){
                        enchantmentList.forEach(item->item.afterGetProjectileDamageSource(tgs));
                    }
                    // 执行附魔效果
                    // other mods progress
                    List<IDamageSourceInitProgress> jointList = ModsJointRegistrar.getJointList(IDamageSourceInitProgress.class);
                    jointList.forEach(item->item.progress(tgs));
                    if (this.itemStack!=null){
                        Map<Enchantment, Integer> enchantments = EnchantmentHelper.getEnchantments(this.itemStack);
                        // 执行附魔效果
                        enchantmentList.forEach(e->e.afterDamageSourceInit(tgs));
                    }
                    // 使dmg与orDmg数值保持一致
                    dmg = tgs.getOriginalDamage();
