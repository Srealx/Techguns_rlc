	...
	public boolean consumeAmmo(){
		...
		if (!gunstack.isEmpty()){
			...
			if (gun.getCurrentAmmo(gunstack)>=1){
				...
				if (!world.isRemote){
					Map<Enchantment, Integer> enchantments = EnchantmentHelper.getEnchantments(gunstack);
					enchantments.forEach((k,v)->{
						if (k instanceof GenericGunEnchantment){
							((GenericGunEnchantment) k).afterAmmoUsed((GenericGun) gunstack.getItem(),gunstack,1);
						}
					});
				}
				return true;
