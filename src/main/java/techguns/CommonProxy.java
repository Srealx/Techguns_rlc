...
import techguns.enchantment.ModEnchantedBooks;
...
public abstract class CommonProxy implements ITGInitializer {
	...
	public void init(FMLInitializationEvent event) {
		...
		// 添加到原版附魔书分类
		Items.ENCHANTED_BOOK.getSubItems(
				CreativeTabs.SEARCH,
				NonNullList.create()
		);
		ItemEnchantedBook.addEnchantment(
				ModEnchantedBooks.COMMON_FIREPOWER_V,
				new EnchantmentData(TGEnchantments.COMMON_GUNS_FIRE_POWER_ENCHANTMENT, 1)
		);
		// 注册到Forge注册表
		ForgeRegistries.ITEMS.register(
				new ItemEnchantedBookSpecial(ModEnchantedBooks.COMMON_FIREPOWER_V)
						.setRegistryName("common_firepower_v")
		);
	...
	private static class ItemEnchantedBookSpecial extends ItemEnchantedBook {
		public ItemEnchantedBookSpecial(ItemStack stack) {
			this.setCreativeTab(CreativeTabs.COMBAT);
			this.setUnlocalizedName(stack.getDisplayName());
		}
	}
	...
	public static void registerEnchantment(RegistryEvent.Register<Enchantment> event) {
		Techguns.instance.enchantments.registerEnchantments(event);
	}
