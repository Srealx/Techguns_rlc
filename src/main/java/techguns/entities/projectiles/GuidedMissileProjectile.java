...
public class GuidedMissileProjectile extends RocketProjectile{
	...
	public GuidedMissileProjectile(World par2World, EntityLivingBase p, float damage, float speed, int TTL,
		...
		super(par2World, p, damage, speed, TTL, spread, dmgDropStart, dmgDropEnd, dmgMin, penetration, blockdamage, leftGun,
				radius, 0.0f,stack,bulletGenericDataBean);
		this.target = target;
		this.damageUuid = bulletGenericDataBean.getDamageUuid();
	}
...
//		epc.lockOnTicks = 0;
	...
	protected void explodeRocket() {
		if (!this.world.isRemote){
			...
			TGExplosion explosion = new TGExplosion(world, this.shooter, this, posX, posY, posZ, this.damage, this.damageMin, this.damageDropStart, this.damageDropEnd, this.blockdamage?0.25:0.0,sourceItemStack,penetration,this.damageUuid);
...
//	}
	...
	public static class Factory implements IChargedProjectileFactory<GuidedMissileProjectile> {
		...
		public GuidedMissileProjectile createProjectile(GenericGun gun, World world, EntityLivingBase p, float damage, float speed, int TTL, float spread, float dmgDropStart, float dmgDropEnd,
			...
			if (target != null) {
				return new GuidedMissileProjectile(world,p,damage,speed,TTL,spread,dmgDropStart,dmgDropEnd,dmgMin,penetration,blockdamage,firePos,radius, target,stack,bulletGenericDataBean);
			}else {
				return new GuidedMissileProjectile(world,p,damage,speed,TTL,spread,dmgDropStart,dmgDropEnd,dmgMin,penetration,blockdamage,firePos,radius,0.01f,stack,bulletGenericDataBean);
			}
		}
		...
		public GuidedMissileProjectile createChargedProjectile(World world, EntityLivingBase p, float damage,
				...
				boolean blockdamage, EnumBulletFirePos firePos, float radius, double gravity, float charge, int ammoConsumed,ItemStack stack, BulletGenericDataBean bulletGenericDataBean) {
//			if (p instanceof EntityPlayer) {
//				TGExtendedPlayer epc = TGExtendedPlayer.get((EntityPlayer)p);
//				epc.lockOnEntity = null;
//				epc.lockOnTicks = 0;
