 ...
 * 穿甲弹(提供额外0.3的护甲穿透)
 */
public class GenericProjectilePenetrate extends GenericProjectile {

	public GenericProjectilePenetrate(World worldIn) {
		super(worldIn);
	}

	public GenericProjectilePenetrate(World worldIn, double posX, double posY, double posZ, float yaw, float pitch,
	...
	public GenericProjectilePenetrate(World par2World, EntityLivingBase p, float damage, float speed, int TTL,
	...
	public static class Factory implements IProjectileFactory<GenericProjectile>{
		...
		@Override
		public GenericProjectilePenetrate createProjectile(GenericGun gun, World world, EntityLivingBase p,
														   float damage, float speed, int TTL, float spread, float dmgDropStart, float dmgDropEnd, float dmgMin,
			...
			return new GenericProjectilePenetrate(world, p, mod.getDamage(damage), speed, TTL, spread, dmgDropStart, dmgDropEnd, mod.getDamage(dmgMin), pen.getPen(penetration), blockdamage, firePos,gunLevel,stack);
		}

		@Override
		public DamageType getDamageType() {
			return DamageType.PENETRATE;
		}
