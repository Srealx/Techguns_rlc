...
public class RocketProjectileNuke extends RocketProjectile {
	...
	protected void explodeRocket() {
		if (!this.world.isRemote){
			...
			TGExplosion explosion = new TGExplosion(world, this.shooter, this, posX, posY, posZ, this.damage, this.damageMin, this.damageDropStart, this.damageDropEnd, this.blockdamage?0.5:0.0,sourceItemStack,penetration,this.damageUuid);
	...
	protected TGDamageSource getProjectileDamageSource() {
		...
		afterGetProjectileDamageSource(dmgsrc);
		return dmgsrc;
	}

	public static class Factory implements IProjectileFactory<RocketProjectileNuke> {

		protected DamageModifier mod = new DamageModifier().setDmg(2.5f, 0f).setRadius(5f, 0f).setRange(5f, 0f);
		...
		public RocketProjectileNuke createProjectile(GenericGun gun, World world, EntityLivingBase p, float damage, float speed, int TTL, float spread, float dmgDropStart, float dmgDropEnd,
			...
			return new RocketProjectileNuke(world,p,mod.getDamage(damage),speed,TTL,spread,mod.getRange(dmgDropStart),mod.getRange(dmgDropEnd),mod.getDamage(dmgMin),penetration,blockdamage,firePos,mod.getRadius(radius),gravity,stack,bulletGenericDataBean);
		}
