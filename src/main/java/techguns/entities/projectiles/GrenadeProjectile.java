...
public class GrenadeProjectile extends GenericProjectile {
	...
	public GrenadeProjectile(World par2World, EntityLivingBase p, float damage, float speed, int TTL, float spread, float dmgDropStart, float dmgDropEnd, float dmgMin, float penetration, boolean blockdamage, EnumBulletFirePos leftGun, double gravity, float radius, int bounces, ItemStack stack,BulletGenericDataBean bulletGenericDataBean) {
	...
	protected void explode(){
		if (!this.world.isRemote){
			...
			*/
			TGExplosion exp = new TGExplosion(world, shooter, this, posX, posY, posZ, this.damage, this.damageMin, this.damageDropStart, this.damageDropEnd, 0,sourceItemStack,penetration,this.damageUuid);
	...
	public static class Factory implements IGrenadeProjectileFactory<GrenadeProjectile> {
		...
		public GrenadeProjectile createProjectile(World world, EntityLivingBase p, float damage, float speed, int TTL, float spread, float dmgDropStart, float dmgDropEnd, float dmgMin, float penetration, boolean blockdamage,
												  EnumBulletFirePos leftGun, float radius, double gravity, float charge, int bounces, ItemStack stack, BulletGenericDataBean bulletGenericDataBean) {
			return new GrenadeProjectile(world, p, damage, speed, TTL, spread, dmgDropStart, dmgDropEnd, dmgMin, penetration, blockdamage, leftGun, gravity, radius, bounces,stack,bulletGenericDataBean);
		}
