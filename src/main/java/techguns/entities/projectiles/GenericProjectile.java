...
import techguns.joint.IceAndFireJoint;
...
public class GenericProjectile extends Entity implements IProjectile, IEntityAdditionalSpawnData {
	...
	protected ItemStack sourceItemStack;
	...
	/**
	 * 该参数用于标记霰弹、再生子弹、贯穿子弹、爆炸等伤害为原子化,用于将多伤害统一计算暴击等
	 */
	protected String damageUuid;
	...
	public void setDamageUuid(String uuid){
		this.damageUuid = uuid;
	}
...
//	}
	...
	public GenericProjectile(World par2World, EntityLivingBase p, double x, double y, double z ,float yaw, float pitch  , float damage, float speed, int TTL, float spread,
							 float dmgDropStart, float dmgDropEnd, float dmgMin, float penetration, boolean blockdamage, EnumBulletFirePos firePos,ItemStack stack) {
		...
		this(par2World);
		this.sourceItemStack = stack;
		this.shooter = p;
		this.initProjectile(par2World, x,y,z, yaw, pitch, damage, speed,
	...
	private void initProjectile(World worldIn, double posX, double posY, double posZ, float yaw, float pitch, float damage,
		...
		float toYaw = yaw + (float) (spread - (2 * Math.random() * spread)) * 40.0f;
		float toPitch = pitch + (float) (spread - (2 * Math.random() * spread)) * 40.0f;
		this.setLocationAndAngles(posX, posY, posZ, toYaw,
				toPitch);
	...
	protected void onHitBlock(RayTraceResult raytraceResultIn){
		boolean deadFlag = this.hitBlock(raytraceResultIn);
		if (Boolean.TRUE.equals(deadFlag)){
			this.setDead();
		}
	}
	...
	protected TGDamageSource getProjectileDamageSource() {
		...
		if (this.knockBackRate!=null){
			src.setKnockback(this.knockBackRate);
		}else {
			src.setNoKnockback();
		}
	...
	protected void afterGetProjectileDamageSource(TGDamageSource src){
		if (this.sourceItemStack!=null){
			Map<Enchantment, Integer> enchantments = EnchantmentHelper.getEnchantments(this.sourceItemStack);
			// 过滤附魔并对附魔进行排序
			List<GenericGunEnchantment> enchantmentList = enchantments.keySet().stream().filter(item -> item instanceof GenericGunEnchantment)
					.map(item -> (GenericGunEnchantment) item).sorted(Comparator.comparingInt(GenericGunEnchantment::getSort)).collect(Collectors.toList());
			// 执行附魔效果
			enchantmentList.forEach(k->k.afterGetProjectileDamageSource(src));
		}
	}
	...
	public static class Factory implements IProjectileFactory<GenericProjectile> {
		...
		public GenericProjectile createProjectile(GenericGun gun, World world, EntityLivingBase p, float damage, float speed, int TTL, float spread, float dmgDropStart, float dmgDropEnd,
			...
			GenericProjectile genericProjectile = new GenericProjectile(world,p,damage,speed,TTL,spread,dmgDropStart,dmgDropEnd,dmgMin,penetration,blockdamage,firePos,stack);
			genericProjectile.setDamageUuid(bulletGenericDataBean.getDamageUuid());
			genericProjectile.setKnockBackRate(bulletGenericDataBean.getKnockBackRate());
