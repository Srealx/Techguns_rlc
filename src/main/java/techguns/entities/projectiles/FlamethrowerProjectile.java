...
import techguns.entities.projectiles.effect.FlameThrowerBoomExplosion;
...
public class FlamethrowerProjectile extends GenericProjectile implements ILightProvider {
	...
	protected void onHitEffect(EntityLivingBase ent, RayTraceResult rayTraceResult) {
		...
		new FlameThrowerBoomExplosion(ent.world,this.shooter,position.getX(),position.getY(),position.getZ(),1.8f,ent).doExplosionA();

		TGPackets.network.sendToAllAround(new PacketSpawnParticle("FlamethrowerImpact",position.getX(),position.getY(),position.getZ(),4f), TGPackets.targetPointAroundEnt(this, 50.0f));

	}

	@Override
	protected boolean hitBlock(RayTraceResult mop) {
		...
		// 给目标周围的敌人都上火
		new FlameThrowerBoomExplosion(this.world,this.shooter,blockPos.getX(),blockPos.getY(),blockPos.getZ(),1.8f,null).doExplosionA();
...
//		Techguns.proxy.createFX("FlamethrowerImpact", this.world, hitVec.x, hitVec.y, hitVec.z, 0f, 0f, 0f);
		...
		TGPackets.network.sendToAllAround(new PacketSpawnParticle("FlamethrowerImpact", hitVec.x,hitVec.y,hitVec.z,4f), TGPackets.targetPointAroundEnt(this, 50.0f));
	...
	public static class Factory implements IProjectileFactory<FlamethrowerProjectile> {
		...
		public FlamethrowerProjectile createProjectile(GenericGun gun, World world, EntityLivingBase p, float damage, float speed, int TTL, float spread, float dmgDropStart, float dmgDropEnd,
			...
			return new FlamethrowerProjectile(world,p,damage,speed,TTL,spread,dmgDropStart,dmgDropEnd,dmgMin,penetration,blockdamage,firePos,gravity,stack);
		}
