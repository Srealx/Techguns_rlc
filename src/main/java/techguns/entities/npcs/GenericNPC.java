	...
	protected boolean hasAimedBowAnim;
	  ...
	  /*  @Override
	    ...
	    public void attackEntityWithRangedAttack(EntityLivingBase target, float distance)
	    {
	    	...
	    	if (this.getHeldItemMainhand() == null) {
	    		...
	    		if (gun instanceof techguns.items.guns.GenericGun) {
					...
					IAttributeInstance entityAttribute = this.getEntityAttribute(SharedMonsterAttributes.ATTACK_DAMAGE);
					if (entityAttribute!=null){
						...
						for (AttributeModifier item : modifiers) {
							...
							switch (operation){
								...
								case 0:{
									// 固伤: 每1点加7%枪械伤害
									extraDamageRate+=0.07f*item.getAmount();
								}break;
								// 类型1表示基础伤害百分比加成
								case 1:{
									// 收益 50%
									extraDamageRate+=0.5f*item.getAmount();
								}break;
								// 类型2表示总伤害百分比加成
								case 2:{
									// 收益 60%
									extraDamageRate+=0.6f*item.getAmount();
								}break;
								default:break;
							}
						}
					}
					dmg *= this.getShootGunDamageRate() * extraDamageRate;
		...
		public void addEnchantmentForGun(ItemStack itemStack){
			...
			// 火力附魔的概率
			int randomFirePower = new Random().nextInt(20)+1;
			switch (randomFirePower){
				case 1:itemStack.addEnchantment(TGEnchantments.LOWER_GUNS_FIRE_POWER_ENCHANTMENT,1);break;
				case 9:itemStack.addEnchantment(TGEnchantments.LOWER_GUNS_FIRE_POWER_ENCHANTMENT,2);break;
				case 17:itemStack.addEnchantment(TGEnchantments.LOWER_GUNS_FIRE_POWER_ENCHANTMENT,3);break;
				default:break;
			}
			// 快速换弹附魔的概率
