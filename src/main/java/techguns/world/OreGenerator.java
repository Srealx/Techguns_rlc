...
//				}
				// TODO 修改了生成数据
				if (TGConfig.doOreGenCopper){
					this.addOreSpawn(TGBlocks.TG_ORE.getDefaultState().withProperty(TGBlocks.TG_ORE.ORE_TYPE, EnumOreType.ORE_COPPER), world, random, chunkX*16, chunkZ*16, 16, 16, 5+random.nextInt(3), 10, 5, 70);
				}
				if(TGConfig.doOreGenTin) {
					this.addOreSpawn(TGBlocks.TG_ORE.getDefaultState().withProperty(TGBlocks.TG_ORE.ORE_TYPE, EnumOreType.ORE_TIN), world, random, chunkX*16, chunkZ*16, 16, 16, 4+random.nextInt(3), 8, 10, 60);
				}
				if (TGConfig.doOreGenLead) {
					this.addOreSpawn(TGBlocks.TG_ORE.getDefaultState().withProperty(TGBlocks.TG_ORE.ORE_TYPE, EnumOreType.ORE_LEAD), world, random, chunkX*16, chunkZ*16, 16, 16, 4+random.nextInt(2), 7, 10, 60);
				}
				this.addOreSpawn(TGBlocks.TG_ORE.getDefaultState().withProperty(TGBlocks.TG_ORE.ORE_TYPE, EnumOreType.ORE_TUNGSTEN), world, random, chunkX*16, chunkZ*16, 16, 16, 4+random.nextInt(2), 7, 3, 30);
			}
	}
