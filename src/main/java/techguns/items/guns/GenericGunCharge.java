			...
			if (world.isRemote) {
				...
				if (cp.player_zoom != 1.0f) {
					...
					if (!player.isSprinting()){
						...
						FovTask.fovChange((int)Minecraft.getMinecraft().gameSettings.fovSetting,(int)nextFov);
						...
						PlayerDataCache.PLAYER_ZOOM_DATA.put(player.getUniqueID(),zoomCacheDataBean);
					}
				}
			}
				...
				if (InventoryUtil.consumeAmmoPlayer(player, this.ammoType.getAmmo(this.getCurrentAmmoVariant(item)))) {
					...
					for (Map.Entry<Enchantment, Integer> entry : enchantments.entrySet()) {
						...
						if (enchantment instanceof GenericGunEnchantment){
							reloadTime = ((GenericGunEnchantment) enchantment).beforeReloadAmmo(item, reloadTime);
						}
					}
					
//					extendedPlayer.setFireDelay(handIn, reloadTime-this.minFiretime);
					extendedPlayer.setFireDelay(handIn, reloadTime);
	...
	public void spawnChargedProjectile(final World world, final EntityLivingBase player, ItemStack itemStack, float spread, float charge, int ammoConsumed, EnumBulletFirePos firePos) {
		...
		GenericProjectile proj = fact.createChargedProjectile(world, player, damage, speed, this.getScaledTTL(), spread, this.damageDropStart, damageDropEnd, this.damageMin, penetration, getDoBlockDamage(player), firePos, radius, gravity, charge, ammoConsumed,itemStack,bulletGenericDataBean);
