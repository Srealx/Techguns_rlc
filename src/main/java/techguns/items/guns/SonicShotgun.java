...
import techguns.joint.IProjectileCreateProgress;
...
public class SonicShotgun extends GenericGun {
	...
	protected void spawnProjectile(World world, EntityLivingBase player, ItemStack itemstack, float spread,
			float offset, float damagebonus, EnumBulletFirePos firePos, Entity target,String damageUuid) {
		...
		SonicShotgunProjectile proj = new SonicShotgunProjectile(world, player, damage*damagebonus, speed, this.ticksToLive, 0f, this.damageDropStart, this.damageDropEnd, this.damageMin*damagebonus, this.penetration, getDoBlockDamage(player), firePos,itemstack);
		...
		for (int j = 1; j <= rings; j++) {
			...
			for (int i = 0; i < (count*j); i++) {
				...
				proj = new SonicShotgunProjectile(world, player, damage*damagebonus, speed, this.ticksToLive, 0f, this.damageDropStart, this.damageDropEnd, this.damageMin*damagebonus, this.penetration, getDoBlockDamage(player), firePos,itemstack);
				proj.entitiesHit = entitiesHit;
