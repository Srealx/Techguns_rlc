...
public class PowerHammer extends GenericGunMeleeCharge {
	...
	public void spawnChargedProjectile(final World world, final EntityLivingBase player, ItemStack itemStack, float spread, float charge, int ammoConsumed, EnumBulletFirePos firePos) {
		...
		GenericProjectile proj = fact.createChargedProjectile(world, player, damage+extraDmg, speed, this.ticksToLive, spread, this.damageDropStart, damageDropEnd, this.damageMin+extraDmg, penetration, getDoBlockDamage(player), firePos, radius, gravity, charge, ammoConsumed,itemStack,bulletGenericDataBean);
