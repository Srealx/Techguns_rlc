...
public interface IChargedProjectileFactory<T extends GenericProjectile> extends IProjectileFactory<T> {
	 ...
	 public T createChargedProjectile(World world, EntityLivingBase p, float damage, float speed, int TTL, float spread,
									  float dmgDropStart, float dmgDropEnd, float dmgMin, float penetration, boolean blockdamage, EnumBulletFirePos firePos, float radius, double gravity, float charge, int ammoConsumed, ItemStack stack,BulletGenericDataBean bulletGenericDataBean);
