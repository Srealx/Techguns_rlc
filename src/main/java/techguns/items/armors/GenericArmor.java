...
public class GenericArmor extends ItemArmor implements ISpecialArmor , IItemTGRenderer{
	...
	public GenericArmor setArmorValue(int armorValue){
...
//			this.addArmorvaluesInformation(item, list);
			...
			if(this.getBonus(TGArmorBonus.EXTRA_HEART)>0.0f){
				...
				list.add(trans("armorTooltip.movespeed")+": +"+String.format("%.2f",this.getBonus(TGArmorBonus.SPEED)*100.0f) +"%");
			} if(this.getBonus(TGArmorBonus.JUMP)>0.0f){
				list.add(trans("armorTooltip.jumpheight")+": +"+ String.format("%.2f",this.getBonus(TGArmorBonus.JUMP) * 5));
			} if(this.getBonus(TGArmorBonus.FALLDMG)>0.0f){
				list.add(trans("armorTooltip.falldamage")+": -"+ String.format("%.2f",this.getBonus(TGArmorBonus.FALLDMG)*100.0f)+"%");
			} if(this.getBonus(TGArmorBonus.FREEHEIGHT)>0.0f){
				list.add(trans("armorTooltip.fallheight")+": -"+String.format("%.2f",this.getBonus(TGArmorBonus.FREEHEIGHT)));
			} if(this.getBonus(TGArmorBonus.BREAKSPEED)>0.0f){
				list.add(trans("armorTooltip.miningspeed")+": +"+String.format("%.2f",this.getBonus(TGArmorBonus.BREAKSPEED)*100.0f)+"%");
			} if(this.getBonus(TGArmorBonus.BREAKSPEED_WATER)>0.0f){
				list.add(trans("armorTooltip.underwatermining")+": +"+String.format("%.2f",this.getBonus(TGArmorBonus.BREAKSPEED_WATER)*100.0f)+"%");
			} if(this.getBonus(TGArmorBonus.KNOCKBACK_RESISTANCE)>0.0f){
				list.add(trans("armorTooltip.knockbackresistance")+": +"+String.format("%.2f",this.getBonus(TGArmorBonus.KNOCKBACK_RESISTANCE)*100.0f)+"%");
			...
			} if (this.getBonus(TGArmorBonus.GUN_ACCURACY)>0.0f){
				list.add(trans("armorTooltip.gunaccuracy")+": +"+String.format("%.2f",this.getBonus(TGArmorBonus.GUN_ACCURACY)*100.0f)+"%");
			}
		} else {
